...
function createOrder($conn, $user_id, $total_amount, $payment_method, $address_id, $cart_items) {
    ...
    // Kiểm tra nếu $total_amount là NULL, thay thế bằng 0
    if ($total_amount === NULL) {
        $total_amount = 0;  // Nếu là NULL, gán giá trị mặc định 0
    }

    // Kiểm tra nếu có mã giảm giá
    $discount_code = $_SESSION['ma_giam_gia'] ?? ''; // Nếu có mã giảm giá thì lấy ra
    $discount_percent = 0;
    $ma_giam_gia_id = NULL;

    if (!empty($discount_code)) {
        // Kiểm tra mã giảm giá
        $discount_info = validateDiscountCode($conn, $discount_code, $total_amount);
        if ($discount_info['valid']) {
            $discount_percent = $discount_info['discount'];
            $ma_giam_gia_id = $discount_info['data']['id'];  // Lấy ID của mã giảm giá hợp lệ
            $total_amount = $total_amount * (1 - $discount_percent / 100); // Áp dụng giảm giá
        }
    }

    // Tạo câu lệnh SQL để thêm đơn hàng
    $sql = "INSERT INTO don_hang (khach_hang_id, tong_tien, hinh_thuc_thanh_toan, trang_thai, ngay_dat, ma_giam_gia_id) 
    ...
    $stmt->bind_param("dssi", $user_id, $total_amount, $payment_method, $ma_giam_gia_id);  // Cập nhật bind_param
    ...
    return $order_id;  // Trả về id của đơn hàng đã tạo
}
