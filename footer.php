<!-- Section: Địa chỉ cửa hàng -->
<section class="my-5">
    <div class="container bg-black text-white p-5 rounded-4 border border-white store-box">
        <h4 class="text-center fw-bold mb-4">Hệ Thống Cửa Hàng</h4>
        <div class="row g-4">
            <!-- Cột trái -->
            <div class="col-md-6">
                <div class="store-item">
                    <div class="store-title">TopZone Đại Lộ Lợi</div>
                    <div class="store-address">Đường Đại Lộ Lợi, Phường Phú Sơn, TP Thanh Hóa</div>
                    <div class="store-pay">Hỗ trợ Apple Pay</div>
                </div>
                <div class="store-item">
                    <div class="store-title">TopZone 133 Xô Viết</div>
                    <div class="store-address">133 Xô Viết Nghệ Tĩnh, Q. Bình Thạnh, TP HCM</div>
                    <div class="store-pay">Hỗ trợ Apple Pay</div>
                </div>
                <div class="store-item">
                    <div class="store-title">TopZone Trần Phú</div>
                    <div class="store-address">4/23 Trần Phú, TP Đà Lạt, Lâm Đồng</div>
                    <div class="store-pay">Hỗ trợ thanh toán qua Apple Pay</div>
                </div>
            </div>

            <!-- Cột phải -->
            <div class="col-md-6">
                <div class="store-item">
                    <div class="store-title">TopZone Cách Mạng Tháng 8</div>
                    <div class="store-address">287A CMT8, P12, Quận 10, TP HCM</div>
                    <div class="store-pay">Hỗ trợ thanh toán qua Apple Pay</div>
                </div>
                <div class="store-item">
                    <div class="store-title">TopZone Nguyễn Xiển</div>
                    <div class="store-address">125 Nguyễn Xiển, P. Long Thành Mỹ, TP Thủ Đức</div>
                    <div class="store-pay">Hỗ trợ thanh toán qua Apple Pay</div>
                </div>
                <div class="store-item">
                    <div class="store-title">TopZone Yên Viên</div>
                    <div class="store-address">Thôn Lê Công, Xã Yên Viên, Huyện Gia Lâm, TP HN</div>
                    <div class="store-pay">Hỗ trợ thanh toán qua Apple Pay</div>
                </div>
            </div>
        </div>

        <div class="text-center mt-4">
            <a href="#" class="store-pay">Xem thêm 79 cửa hàng</a>
        </div>
    </div>
</section>


<!-- Section: Lợi ích -->
<section class="bg-dark text-white py-4">
    <div class="container-fluid">
        <div class="row text-center justify-content-center">

            <div class="col-6 col-md-3 mb-3">
                <i class="fas fa-check-circle mb-2" style="font-size: 26px;"></i>
                <p class="m-0 fw-semibold" style="font-size: 18px;">Mẫu mã đa dạng,<br>chính hãng</p>
            </div>

            <div class="col-6 col-md-3 mb-3">
                <i class="fas fa-truck mb-2" style="font-size: 26px;"></i>
                <p class="m-0 fw-semibold" style="font-size: 18px;">Giao hàng toàn quốc</p>
            </div>

            <div class="col-6 col-md-3 mb-3">
                <i class="fas fa-shield-alt mb-2" style="font-size: 26px;"></i>
                <p class="m-0 fw-semibold" style="font-size: 18px;">Bảo hành có cam kết<br>tới 12 tháng</p>
            </div>

            <div class="col-6 col-md-3 mb-3">
                <i class="fas fa-sync-alt mb-2" style="font-size: 26px;"></i>
                <p class="m-0 fw-semibold" style="font-size: 18px;">Có thể đổi trả tại<br>Thegioididong và DienmayXANH
                </p>
            </div>

        </div>
    </div>
</section>



<!-- Section: Footer -->
<!-- Footer phần 1: Logo -->
<div class="footer-logo text-white d-flex align-items-center" style="height: 100px; background-color: #000;">
    <div class="col-md-1 mb-4"></div>

    <div class="container-fluid ps-5 d-flex align-items-center gap-4">
        <img src="img/logo-topzone-removebg-preview.png" alt="TopZone Logo" style="height: 70px;">
        <img src="img/tao1.png" alt="Apple Reseller" style="height: 80px;">
    </div>
</div>

<!-- Footer phần 2: Thông tin -->
<footer class="footer-info bg-black text-white pt-5">
    <div class="container-fluid px-5">
        <div class="row text-start text-md-start">
            <div class="col-md-1 mb-4"></div>

            <!-- Cột 1 -->
            <div class="col-md-2 mb-4">
                <h6 class="fw-bold">Tổng đài</h6>
                <p class="mb-1">Mua hàng: <a href="tel:19009696.42"
                        class="text-info text-decoration-none fw-normal">1900.9696.42</a> (8:00 – 21:30)</p>
                <p class="mb-1">Khiếu nại: <a href="tel:19009868.43"
                        class="text-info text-decoration-none fw-normal">1900.9868.43</a> (8:00 – 21:30)</p>
                <h6 class="mt-4 fw-bold">Kết nối với chúng tôi</h6>
                <div class="d-flex gap-3 mt-2">
                    <a href="#" class="circle-icon"><i class="fab fa-facebook-f"></i></a>
                    <a href="#" class="circle-icon"><i class="fab fa-youtube"></i></a>
                    <a href="#" class="circle-icon"><i class="fas fa-phone-alt"></i></a>
                </div>

            </div>

            <!-- Cột 2 -->
            <div class="col-md-2   mb-4">
                <h6 class="fw-bold">Hệ thống cửa hàng</h6>
                <p><a href="#" class="text-white">Xem 85 cửa hàng</a></p>
                <p><a href="#" class="text-white">Nội quy cửa hàng</a></p>
                <p><a href="#" class="text-white">Chất lượng phục vụ</a></p>
                <p><a href="#" class="text-white">Chính sách bảo hành & đổi trả</a></p>
            </div>

            <!-- Cột 3 -->
            <div class="col-md-2 mb-4">
                <h6 class="fw-bold">Hỗ trợ khách hàng</h6>
                <p><a href="#" class="text-white">Điều kiện giao dịch chung</a></p>
                <p><a href="#" class="text-white">Hướng dẫn mua online</a></p>
                <p><a href="#" class="text-white">Chính sách giao hàng</a></p>
                <p><a href="#" class="text-white">Hướng dẫn thanh toán</a></p>
            </div>

            <!-- Cột 4 -->
            <div class="col-md-2 mb-4">
                <h6 class="fw-bold">Về TopZone</h6>
                <p><a href="#" style="color: #00bfff; font-weight: 500;">Tích điểm Quà tặng VIP</a></p>
                <p><a href="#" class="text-white">Giới thiệu TopZone</a></p>
                <p><a href="#" class="text-white">Chính sách dữ liệu cá nhân</a></p>
                <p><a href="#" class="text-white">Xem bản mobile</a></p>
            </div>

            <!-- Cột 5 -->
            <div class="col-md-2 mb-4">
                <h6 class="fw-bold">Trung tâm bảo hành TopCare</h6>
                <p><a href="login.php" class="text-white text-decoration-none">Giới thiệu TopCare</a></p>

            </div>
            <div class="col-md-1 mb-4"></div>

        </div>
    </div>


    <!-- Footer phần cuối -->
    <div class="bg-dark text-secondary pt-3 pb-4">
        <div class="container-fluid px-5 small">
            <hr class="border-secondary">
            <p class="mb-0">
                © 2018. Công ty cổ phần Thế Giới Di Động. GPĐKKD: 0303217354 do sở KH & ĐT TP.HCM cấp ngày
                02/01/2007.<br>
                Địa chỉ: 128 Trần Quang Khải, P.Tân Định, Q.1, TP. Hồ Chí Minh. Điện thoại: 028 38125960. <br>
                Địa chỉ liên hệ và gửi chứng từ: Lô T2-1.2, Đường D1, Đ. D1, P.Tân Phú, TP.Thủ Đức, TP.Hồ Chí Minh. <br>
                Chịu trách nhiệm nội dung: Huỳnh Văn Tốt. Email: hotrotdmt@thegioididong.com
            </p>
        </div>
    </div>
</footer>

</html>